...
class RAMSubjectData:
    ...
    def load_electrode_info(self, bipolar = False):
        if not bipolar:        
            self.electrodes = load_electrode_info(self.subject, self.montage, bipolar=False)
        elif bipolar:
            self.electrodes_bipolar = load_electrode_info(self.subject, self.montage, bipolar=True)
    
    def clean_electrodes(self):
        electrodes_d = self.electrodes[self.electrodes['type'] ==  'D']
        self.electrodes_depth = clean_electrodes(electrodes_d)

    @property
    def electrodes_in_roi(self):
        pass
    ...
    def load_subjects(self):
        ...
        def load_subject(task, subject_id, montage_id):
            try:
                ...
                subject.load_electrode_info(bipolar=True)
...
def clean_electrodes(elec_info, elec_column1='stein.region', elec_column2='ind.region',
        ...
        if roi_dict is None:
            roi_dict = {'Hippocampus'      : ['Left CA1', 'Left CA2', 'Left CA3', 'Right CA1', 'Right CA2',
                                             'Right CA3', 'hippocampus', 'Hippocampus', 'Left Hippocampus', ' Right Hippocampus'],
                        'DentateGyrus'     : ['Left DG', 'Right DG'],
                        'Subiculum'        : ['Left Sub', 'Right Sub'],
                        'EntorhinalCortex' : ['Right EC', 'Left EC', 'entorhinal', 'EC'],
                        'MTL'              : ['Left PRC', 'Right PRC', 'Right PHC', 'Left PHC'],
        ...
        # make new DF
        elec_df = elec_info[['contact', 'label', 'description', 'type', 'avg.x', 'avg.y', 'avg.z' ]].copy()
        elec_df['region'] = regions
        elec_df['area'] = elec_region_list
        elec_df['hemi'] = elec_hemi_list
